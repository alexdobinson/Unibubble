...
// 	// Route::get('{param1?}, {param2?}, {param3?}')
	...
	Route::get('looking-for', function() 
	{
		$app = app();

		$params['looking_for'] = true;

		$controller = $app->make('MarketplaceController');

		$controller->callAction($app, $app['router'], 'getListings', $params);
	});

	Route::get('looking-for/{category}/{sub_category?}', function($category, $sub_category=null) 
	{
		$params['looking_for'] = true;
		...
		$controller->callAction($app, $app['router'], 'getListings', $parameters = array('fg' => 'ggfd', 'dfd' => 'dsds'));
	});
